...
import scala.language.{implicitConversions, postfixOps} // silence warnings
...
trait ListImplementation[A] extends List[A] {
  ...
  override def zipRight: List[(A,Int)] = {
    val counter = Iterator.iterate(0)(i => i + 1)
    this.map(a => (a, counter.next()))
  }

  override def partition(pred: A => Boolean): (List[A],List[A]) = (this.filter(y => pred(y)), this.filter(n => !pred(n)))

  override def span(pred: A => Boolean): (List[A],List[A]) = partition(pred)
  ...
  override def reduce(op: (A,A)=>A): A = this match {
    case h :: t => if (t.tail.isEmpty) h else this.foldRight(h)(op)
    case _ => throw new UnsupportedOperationException()
  }

  override def takeRight(n: Int): List[A] = {
    def myfunc(list: List[A], acc: List[A], iterator: Iterator[Int]): List[A] = {
      if (!iterator.hasNext || list.head.isEmpty)
        acc
      else {
        iterator.next()
        myfunc(list.tail.get, acc.append(Cons(list.head.get, Nil())), iterator)
      }
    }

    myfunc(this.reverse(), Nil(), Iterator.iterate(0)(i => i + 1).take(n)).reverse()
  }

  override def collect[B](partialFunction: PartialFunction[A,B]): List[B] =
    this.filter(elem => partialFunction.isDefinedAt(elem)).map(elem => partialFunction.apply(elem))
}
